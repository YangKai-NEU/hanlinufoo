...
$conne->init_conn();
if ($type == "2") {
    $sql = "SELECT * FROM student WHERE s_id NOT IN(SELECT m_sid FROM class_item WHERE m_cid='" . $cid . "')";
} else {
    $sql = "SELECT * FROM class_item,student WHERE m_cid='" . $cid . "' AND m_sid=s_id";
}
