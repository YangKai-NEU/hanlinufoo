...
function newStudent() {
    if (g_class_student == null) {
        g_class_student = new Array();
    }
}
function clickSelectedStudentItem(id) {
    $("#selected_student_" + id).remove();
    for (var i = 0; i < g_class_student.length; i++) {
        if (g_class_student[i] == id) {
            g_class_student.splice(i, 1);
            break;
        }
    }
}
function clickAllStudentItem(id, name) {
    newStudent();
    var i;
    for (i = 0; i < g_class_student.length; i++) {
        if (g_class_student[i] == id) {
            break;
        }
    }
    if (i == g_class_student.length) {
        g_class_student.push(id);
        var item = document.createElement("div");
        item.setAttribute("class", "student_item cur_hand");
        item.setAttribute("id", "selected_student_" + id);
        item.setAttribute("onclick", "clickSelectedStudentItem(" + id + ")");
        item.innerHTML = name;
        $("#selected_student").append(item);
    }
}
function clickClassItem(id) {
    $("#class_table_name").html("");
    $("#selected_student").html("");
    $("#all_student").html("");
    g_class_student = null;
    $("#questions-content").hide();
    $("#class_table").show();
    $("#class_table_bottom").show();
    g_current_work = "class";
    g_current_cid = id;
    $div1.show();
    $.ajax({
        type: "POST",
        url: "./action/student_all_get.class.php",
        data: {},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
            $div1.hide();
            // console.log("students", JSON.stringify(msg));
            if (obj.Code == "1") {
                for (var i = 0; i < obj.Data.length; i++) {
                    var item = document.createElement("div");
                    item.setAttribute("class", "student_item cur_hand");
                    item.setAttribute("onclick", "clickAllStudentItem(" + obj.Data[i].m_id + ",'" + obj.Data[i].m_username + "')");
                    item.innerHTML = obj.Data[i].m_username;
    ...
    $.ajax({
        type: "POST",
        url: "./action/student_from_class_get.class.php",
        data: {cid: id},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
            $div1.hide();
            // console.log("selected_students", JSON.stringify(msg));
            $("#class_table_name").html(obj.Name);
            if (obj.Code == "1") {
                for (var i = 0; i < obj.Data.length; i++) {
                    clickAllStudentItem(obj.Data[i].m_id, obj.Data[i].m_username);
                }
            }
        }
    });
}
function deleteQuestionFromQuiz(id, quizid) {
    $div1.show();
    $.ajax({
        type: "POST",
        url: "./action/quiz_item_from_teacher_delete.class.php",
        data: {id: id},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
...
function publishQuiz(hid) {
    $div1.show();
    $.ajax({
        type: "POST",
        url: "./action/quiz_from_teacher_publish.class.php",
        data: {hid: hid, title: $("#quiz_title").val(), cid: g_final_class},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
            $div1.hide();
...
function setActiveInHomeworkClasses(id) {
    $("#homework_classes .homework_classes_item_click").removeClass("homework_classes_item_click");
    $("#homework_classes_item_" + id).addClass("homework_classes_item_click");
    g_final_class = id;
}
function deleteQuiz(id) {
    ...
    $.ajax({
        type: "POST",
        url: "./action/quiz_from_teacher_delete.class.php",
        data: {hid: id},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
...
function clickHomeworkItem(id) {
    $div1.show();
    $.ajax({
        type: "POST",
        url: "./action/quiz_content_from_teacher_get.class.php",
        data: {hid: id},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
            $div1.hide();
            // console.log("homework_content", JSON.stringify(msg));

            $("#questions-content").show();
            $("#questions-content").html("");
            g_question_position = 0;
            g_page = 0;
            g_current_work = "edit_quiz";
            $("#question_table").hide();
            $("#question_table_bottom").hide();
            $("#class_table").hide();
            $("#class_table_bottom").hide();
            $("#question_add_item").show();
            ...
            g_final_class = obj.Class;
            $("#questions-content").append(title);

            var classes = document.createElement("div");
            classes.setAttribute("class", "homework_classes");
            classes.setAttribute("id", "homework_classes");
            if (g_classes != null) {
                for (var i = 0; i < g_classes.length; i++) {
                    var tmpdiv = document.createElement("div");
                    if (g_final_class == g_classes[i].m_id) {
                        tmpdiv.setAttribute("class", "homework_classes_item_click homework_classes_item cur_hand");
                    } else {
                        tmpdiv.setAttribute("class", "homework_classes_item cur_hand");
                    }
                    tmpdiv.setAttribute("id", "homework_classes_item_" + g_classes[i].m_id);
                    tmpdiv.innerHTML = g_classes[i].m_name;
                    tmpdiv.setAttribute("onclick", "setActiveInHomeworkClasses(" + g_classes[i].m_id + ")");
                    classes.append(tmpdiv);
                }
            }
            $("#questions-content").append(classes);

            for (var i = 0; i < obj.Data.length; i++) {
                if (obj.Data[i].m_type == "1") {
                    ...
                    question_item_title.innerHTML = "<strong>" + (i + 1) + "．</strong></span>" + obj.Data[i].m_question;
                    ...
                    question_item_select_a.innerHTML = "<strong>A．</strong>" + obj.Data[i].m_select_a;
                    var question_item_select_b = document.createElement('div');
                    question_item_select_b.setAttribute("class", "question_select");
                    question_item_select_b.innerHTML = "<strong>B．</strong>" + obj.Data[i].m_select_b;
                    ...
                    question_item_select_c.innerHTML = "<strong>C．</strong>" + obj.Data[i].m_select_c;
                    var question_item_select_d = document.createElement('div');
                    question_item_select_d.setAttribute("class", "question_select");
                    question_item_select_d.innerHTML = "<strong>D．</strong>" + obj.Data[i].m_select_d;
                    ...
                    block3_2_img.setAttribute("onClick", "deleteQuestionFromQuiz(" + obj.Data[i].m_id + "," + id + ")");
            ...
            b3_2_img.setAttribute("onClick", "publishQuiz(" + id + ")");
            b3_2.append(b3_2_img);
            b3.append(b3_1);
            b3.append(b3_2);

            var b5 = document.createElement('div');
            b5.setAttribute("class", "cur_hand inline-block-style");
            var b5_1 = document.createElement('div');
            b5_1.setAttribute("class", "question_bottom_block_1");
            var b5_1_img = document.createElement('img');
            b5_1_img.setAttribute("class", "question_edit_img");
            b5_1_img.setAttribute("src", "./image/question_delete_white.png");
            b5_1.append(b5_1_img);
            var b5_2 = document.createElement('div');
            b5_2.setAttribute("class", "question_bottom_block_2");
            var b5_2_img = document.createElement('a');
            b5_2_img.setAttribute("class", "question_edit_text white_on_deep");
            b5_2_img.innerHTML = "<strong>删除该试卷</strong>";
            b5_2_img.setAttribute("onClick", "deleteQuiz(" + id + ")");
            b5_2.append(b5_2_img);
            b5.append(b5_1);
            b5.append(b5_2);
...
function classTableSubmit() {
    if (g_class_student == null) {
        newStudent();
    }
    // console.log("JSON格式的student", JSON.stringify(g_class_student));
    // console.log("student", g_class_student);
    $div1.show();
    $.ajax({
        type: "POST",
        url: "./action/classes_add.class.php",
        data: {name: $("#class_table_name").html(), data: JSON.stringify(g_class_student), cid: g_current_cid},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
            $div1.hide();
            alert(obj.Msg);
            // console.log("Add Class", JSON.stringify(msg));
            if (obj.Code == "1") {
                $("#class_table").hide();
                $("#class_table_name").html("");
                $("#selected_student").html("");
...
function getMyClassList() {
    $.ajax({
        ...
        success: function (msg) {
            ...
            if (obj.Code == "1") {
                $(".my_class_list_item").remove();
                g_classes = obj.classdata;
...
function getMyHomeworkList() {
    $.ajax({
        type: "POST",
        url: "./action/quiz_from_teacher_get.class.php",
        data: {},
        dataType: "json",
        success: function (msg) {
            var obj = eval(msg);
            // console.log("quizs", JSON.stringify(msg));
            if (obj.Code == "1") {
                $(".my_homework_list_item").remove();
                for (var i = 0; i < obj.Data.length; i++) {
                    var item = document.createElement("div");
                    item.setAttribute("class", "my_homework_list_item cur_hand");
                    if (obj.Data[i].m_publish == 'N') {
                        item.innerHTML = "<strong>未发布</strong> " + obj.Data[i].m_homework_name;
                    } else {
                        item.innerHTML = "<strong>已发布</strong> " + obj.Data[i].m_homework_name;
                    }
                    item.setAttribute("onclick", "clickHomeworkItem(" + obj.Data[i].m_id + ")");
                    var list = document.getElementById("my_homework_list");
                    list.insertBefore(item, list.lastElementChild);
                }
            }
        }
                ...
                if (obj.type == "1") {
                    if (obj.question.length == 0 && g_page == 0) {
                        ...
                        for (var i = 0; i < obj.question.length; i++) {
                            console.log("添加选择题", obj.question[i].question);
                    ...
                    if (obj.question.length == 0 && g_page == 0) {
                        ...
                        for (var i = 0; i < obj.question.length; i++) {
                            console.log("填空题", obj.question[i].question);
                    ...
                    if (obj.question.length == 0 && g_page == 0) {
                        ...
                        for (var i = 0; i < obj.question.length; i++) {
                            console.log("简答题", obj.question[i].question);
...
function updateImgText(can, img, text, id, type, question, select_a, select_b, select_c, select_d, answer, analyse, solution, video) {
    // console.log("can", can);
    console.log("设置题的点击事件（updateImgText）", question);
    if (can == 1) {
        ...
        // console.log("onclick", "addToQuiz('" + img + "', '" + text + "', '" + id + "', '" + type + "', '" + question + "', '" + select_a + "', '" + select_b + "', '" + select_c + "', '" + select_d + "', '" + answer + "', '" + analyse + "', '" + solution + "', '" + video + "'");
        text_.setAttribute("onclick", "addToQuiz('" + img + "','" + text + "','" + id + "','" + type + "','" + formatQuestion(question) + "','" + formatQuestion(select_a) + "','" + formatQuestion(select_b) + "','" + formatQuestion(select_c) + "','" + formatQuestion(select_d) + "','" + formatQuestion(answer) + "','" + formatQuestion(analyse) + "','" + formatQuestion(solution) + "','" + video + "')");
        console.log("题（click1）", "addToQuiz('" + img + "','" + text + "','" + id + "','" + type + "','" + formatQuestion(question) + "','" + formatQuestion(select_a) + "','" + formatQuestion(select_b) + "','" + formatQuestion(select_c) + "','" + formatQuestion(select_d) + "','" + formatQuestion(answer) + "','" + formatQuestion(analyse) + "','" + formatQuestion(solution) + "','" + video + "')");
    } else {
        var img_ = document.getElementById(img);
        ...
        // console.log("onclick", "removeFromQuiz('" + img + "','" + text + "','" + id + "','" + type + "','" + question + "','" + select_a + "','" + select_b + "','" + select_c + "','" + select_d + "','" + answer + "','" + analyse + "','" + solution + "','" + video + "')")；
        text_.setAttribute("onclick", "removeFromQuiz('" + img + "','" + text + "','" + id + "','" + type + "','" + formatQuestion(question) + "','" + formatQuestion(select_a) + "','" + formatQuestion(select_b) + "','" + formatQuestion(select_c) + "','" + formatQuestion(select_d) + "','" + formatQuestion(answer) + "','" + formatQuestion(analyse) + "','" + (solution) + "','" + video + "')");
        console.log("题(click2)", "removeFromQuiz('" + img + "','" + text + "','" + id + "','" + type + "','" + formatQuestion(question) + "','" + formatQuestion(select_a) + "','" + formatQuestion(select_b) + "','" + formatQuestion(select_c) + "','" + formatQuestion(select_d) + "','" + formatQuestion(answer) + "','" + formatQuestion(analyse) + "','" + (solution) + "','" + video + "')");
    }
}
...
function addToQuiz(img, text, id, type, question, select_a, select_b, select_c, select_d, answer, analyse, solution, video) {
    console.log("题（addToQuiz）", question);
    if (g_user == null) {
        alert("您还未登录,请先登录");
        return false;
    }
...
function checkQuestionIfInQuiz(img, text, id, type, question, select_a, select_b, select_c, select_d, answer, analyse, solution, video) {
    console.log("踢踢踢(checkQuestionIfInQuiz)", question);
    if (g_quiz == "") {
        updateImgText(1, img, text, id, type, question, select_a, select_b, select_c, select_d, answer, analyse, solution, video);
        return false;
    }
...
function uploadQuiz() {
    ...
    g_quiz.quiz_name = $("#quiz_title").val();
    $div1.show();
    console.log("题（SUBMIT)", JSON.stringify(g_quiz));
    $.ajax({
        type: "POST",
...
$("#my_homeworks").mouseover(function () {
    $("#my_homework_list").show();
});
$("#my_homeworks").mouseout(function () {
    if (g_state == false) {
        $("#my_homework_list").hide();
    }
});
$("#my_homework_list").mouseover(function () {
    var s = event.fromElement || event.relatedTarget;
    if (!this.contains(s)) {
        g_state = true;
        $("#my_homework_list").show();
...
$("#my_homework_list").mouseout(function () {
    var s = event.toElement || event.relatedTarget;
    if (!this.contains(s)) {
        g_state = false;
        $("#my_homework_list").hide();
...
$("#class_table_back").click(function () {
    $("#class_table").hide();
    $("#selected_student").html("");
    $("#all_student").html("");
    $("#class_table_bottom").hide();
...
$("#class_table_submit").click(function () {
    if ($("#class_table_name").html() == "") {
        alert("请填写班级名称");
        return false;
    }
    classTableSubmit();
});
$("#my_class_add").click(function () {
    $("#questions-content").hide();
    $("#class_table").show();
    $("#class_table_bottom").show();
    g_current_work = "class";
...
$("#class_table_delete").click(function () {
    deleteClass();
});
